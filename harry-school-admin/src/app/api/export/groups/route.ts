...
export const POST = withAuth(async (request: NextRequest, { user }) => {
  try {
    ...
    const exportService = new GroupsExportService(validatedData.organizationId)
    
    // Generate export data
    const exportData = await exportService.exportGroups({
    ...
    const filename = validatedData.filename || `groups_export_${new Date().toISOString().split('T')[0]}`

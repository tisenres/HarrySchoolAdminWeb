...
class AuthCache {
  ...
  /**
   * Get just the organization for a user
   */
  getOrganization(userId: string): string | null {
    // Try to find any cached entry for this user
    const prefix = `auth:${userId}:`
    for (const [key, cached] of this.cache) {
      if (key.startsWith(prefix) && cached.organization) {
        if (Date.now() <= cached.expiresAt) {
          return cached.organization.id || cached.organization
        }
      }
    }
    return null
  ...
  /**
   * Get just the profile for a user
   */
  getProfile(userId: string): any | null {
    // Try to find any cached entry for this user
    const prefix = `auth:${userId}:`
    for (const [key, cached] of this.cache) {
      if (key.startsWith(prefix) && cached.profile) {
        if (Date.now() <= cached.expiresAt) {
  ...
  /**
   * Set organization for a user
   */
  setOrganization(userId: string, organizationId: string): void {
    // Update any existing cached entries for this user
    const prefix = `auth:${userId}:`
    for (const [key, cached] of this.cache) {
